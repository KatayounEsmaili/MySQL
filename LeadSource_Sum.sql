...
DROP TEMPORARY TABLE IF EXISTS
 ...
 select    @X:=concat(@X,' sum( case when potential.leadsource =','''',leadsource,'''' ,' then ifnull(invoice.total,0) else 0 end )  as ', replace(leadsource,' ','_'),  ' ,'  ) 
...
crm.smownerid,concat(users.first_name,'' '',users.last_name) as name,
 ', @X ,'
sum( case when potential.leadsource is null  then ifnull(invoice.total,0) else 0 end ) as NoSource
